...
module.exports = World = cls.Class.extend({
    init: function(id, maxPlayers, websocketServer) {
        ...
        this.onPlayerEnter(function(player) {
            logger.info(player.name + " has joined "+ self.id);
            ...
            var move_callback = function(x, y) {
                logger.debug(player.name + " is moving to (" + x + ", " + y + ").");
    ...
    pushSpawnsToPlayer: function(player, ids) {
        ...
        logger.debug("Pushed "+_.size(ids)+" new spawns to "+player.id);
    },
    ...
    removeEntity: function(entity) {
        ...
        logger.debug("Removed "+ Types.getKindAsString(entity.kind) +" : "+ entity.id);
    },
    ...
    chooseMobTarget: function(mob, hateRank) {
        ...
        if(player && !(mob.id in player.attackers)) {
            ...
            logger.debug(mob.id + " is now attacking " + player.id);
        }
    },
    ...
    logGroupPlayers: function(groupId) {
        logger.debug("Players inside group "+groupId+":");
        ...
        if(entity) {
            ...
            if(!entity.group || (entity.group && entity.group !== groupId)) {
                ...
                if(_.size(oldGroups) > 0) {
                    ...
                    logger.debug("group diff: " + entity.recentlyLeftGroups);
                }
            }
        }
